...
int LogfileManager::writeShutdownInfo (bool writeShutdownTime) {
  ...
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "tick", TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, val.c_str(), val.size()));

  val = basics::StringUtils::itoa(_lastCollectedId);
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "lastCollected", TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, val.c_str(), val.size()));

  val = basics::StringUtils::itoa(_lastSealedId);
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "lastSealed", TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, val.c_str(), val.size()));

  if (writeShutdownTime) {
    std::string const t(getTimeString());
    TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "shutdownTime", TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, t.c_str(), t.size()));
  }
