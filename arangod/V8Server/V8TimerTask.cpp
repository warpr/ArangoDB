...
void V8TimerTask::getDescription (TRI_json_t* json) {
  ...
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "created", created);

  TRI_json_t* cmd = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, _command.c_str(), _command.size());
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "command", cmd);

  TRI_json_t* db = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, _vocbase->_name);
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "database", db);
}
