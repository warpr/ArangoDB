...
bool AgencyCommResult::parseJsonNode (TRI_json_t const* node,
  ...
  if (isDir) {
    if (withDirs) {
      ...
      entry._json  = 0;
      entry._isDir = true;
      _values.emplace(std::make_pair(prefix, entry));
    }
...
AgencyCommResult AgencyComm::sendServerState (double ttl) {
  ...
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "status", TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, status.c_str(), status.size()));
  TRI_InsertAndFreeArrayJson(TRI_UNKNOWN_MEM_ZONE, json, "time", TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, stamp.c_str(), stamp.size()));
...
bool AgencyComm::lockRead (std::string const& key,
                           double ttl,
                           double timeout) {
  TRI_json_t* json = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, "READ", 4);
...
bool AgencyComm::lockWrite (std::string const& key,
                            double ttl,
                            double timeout) {
  TRI_json_t* json = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, "WRITE", 5);
...
AgencyCommResult AgencyComm::uniqid (std::string const& key,
  ...
  while (tries++ < maxTries) {
    ...
    if (result.httpCode() == (int) triagens::rest::HttpResponse::NOT_FOUND) {
      TRI_json_t* json = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, "0", 1);
    ...
    if (it != result._values.end()) {
      ...
      oldJson = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, "0", 1);
    }
...
bool AgencyComm::lock (std::string const& key,
  ...
  while (true) {
    TRI_json_t* oldJson = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, "UNLOCKED", 8);
...
bool AgencyComm::unlock (std::string const& key,
  ...
  while (true) {
    TRI_json_t* newJson = TRI_CreateStringCopyJson(TRI_UNKNOWN_MEM_ZONE, "UNLOCKED", 8);
